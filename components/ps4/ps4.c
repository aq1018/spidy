...
#define ESP_BD_ADDR_HEX_PTR(addr)                                            \
  (unsigned int*)addr + 0, (unsigned int*)addr + 1, (unsigned int*)addr + 2, \
      (unsigned int*)addr + 3, (unsigned int*)addr + 4,                      \
      (unsigned int*)addr + 5
...
/*******************************************************************************
**
** Function         ps4InitWithMac
**
** Description      This sets BT to supplied mac address, then initializes the
**                  bluetooth services to listen for an incoming PS4 controller
**                  connection.
**
**
** Returns          void
**
*******************************************************************************/
void ps4InitWithMac(const char* mac) {
  esp_bd_addr_t addr;
  if (sscanf(mac, ESP_BD_ADDR_STR, ESP_BD_ADDR_HEX_PTR(addr)) !=
      ESP_BD_ADDR_LEN) {
    ESP_LOGE(PS4_TAG, "Could not convert %s\n to a MAC address", mac);
    return;
  }
...
void ps4Cmd(ps4_cmd_t cmd) {
  ...
  hidCommand.data[ps4_control_packet_index_small_rumble] =
      cmd.smallRumble;  // Small Rumble
  hidCommand.data[ps4_control_packet_index_large_rumble] =
      cmd.largeRumble;  // Big rumble
...
**
*******************************************************************************/
void ps4SetConnectionObjectCallback(void* object,
                                    ps4_connection_object_callback_t cb) {
