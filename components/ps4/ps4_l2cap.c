...
static void ps4_l2cap_init_service(const char *name, uint16_t psm,
                                   uint8_t security_id);
static void ps4_l2cap_deinit_service(const char *name, uint16_t psm);
static void ps4_l2cap_connect_ind_cback(BD_ADDR bd_addr, uint16_t l2cap_cid,
                                        uint16_t psm, uint8_t l2cap_id);
static void ps4_l2cap_connect_cfm_cback(uint16_t l2cap_cid, uint16_t result);
static void ps4_l2cap_config_ind_cback(uint16_t l2cap_cid,
                                       tL2CAP_CFG_INFO *p_cfg);
static void ps4_l2cap_config_cfm_cback(uint16_t l2cap_cid,
                                       tL2CAP_CFG_INFO *p_cfg);
...
static const tL2CAP_APPL_INFO dyn_info = {ps4_l2cap_connect_ind_cback,
...
void ps4_l2cap_init_services() {
  ps4_l2cap_init_service("PS4-HIDC", BT_PSM_HIDC, BTM_SEC_SERVICE_FIRST_EMPTY);
  ps4_l2cap_init_service("PS4-HIDI", BT_PSM_HIDI,
...
void ps4_l2cap_deinit_services() {
  ps4_l2cap_deinit_service("PS4-HIDC", BT_PSM_HIDC);
  ps4_l2cap_deinit_service("PS4-HIDI", BT_PSM_HIDI);
}
...
void ps4_l2cap_send_hid(hid_cmd_t *hid_cmd, uint8_t len) {
  ...
  if (!p_buf) {
    ESP_LOGE(PS4_TAG, "[%s] allocating buffer for sending the command failed",
             __func__);
  }
  ...
  memcpy((uint8_t *)(p_buf + 1) + p_buf->offset, (uint8_t *)hid_cmd,
         p_buf->len);
...
static void ps4_l2cap_init_service(const char *name, uint16_t psm,
  ...
  if (!BTM_SetSecurityLevel(false, name, security_id, 0, psm, 0, 0)) {
    ESP_LOGE(PS4_TAG, "%s Registering security service %s failed", __func__,
             name);
    return;
  }
...
**
** Description      This the L2CAP inbound connection indication callback
*function.
**
** Returns          void
**
...
static void ps4_l2cap_connect_ind_cback(BD_ADDR bd_addr, uint16_t l2cap_cid,
  ...
  ESP_LOGI(PS4_TAG,
           "[%s] bd_addr: %s\n  l2cap_cid: 0x%02x\n  psm: %d\n  id: %d",
           __func__, bd_addr, l2cap_cid, psm, l2cap_id);

  /* Send connection pending response to the L2CAP layer. */
  L2CA_CONNECT_RSP(bd_addr, l2cap_id, l2cap_cid, L2CAP_CONN_PENDING,
...
**
*******************************************************************************/
static void ps4_l2cap_disconnect_cfm_cback(uint16_t l2cap_cid,
                                           uint16_t result) {
  ESP_LOGI(PS4_TAG, "[%s] l2cap_cid: 0x%02x\n  result: %d", __func__, l2cap_cid,
           result);
}
